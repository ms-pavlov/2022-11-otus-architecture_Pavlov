...
class QueueLoopThreadImplTest {
    ...
    @BeforeEach
    void setUp() {
        this.loopThread = new QueueLoopThreadImpl(this.commands, this.handlingStrategy, this.commandHandler, this.stopHandler);
    }

    @Test
    @DisplayName("Можно поменять стратегию обработки команд")
    void setCommandHandler() throws InterruptedException {
        List<Command> commandsList = new ArrayList<>();
        when(this.stopHandler.get()).thenAnswer((invocationOnMock) -> commandsList.isEmpty());
        when(this.commands.take()).thenReturn(this.command);
